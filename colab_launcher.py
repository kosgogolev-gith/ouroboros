...
DIAG_SLOW_CYCLE_SEC = _parse_int_cfg(
    get_cfg("OUROBORUS_DIAG_SLOW_CYCLE_SEC", default="20", allow_legacy_secret=True),
    default=20,
    minimum=0,
)
...
# 2) Mount Drive
# ----------------------------
_DRIVE_ROOT_CFG = get_cfg("OUROBOROS_DRIVE_ROOT", default=None)
if _DRIVE_ROOT_CFG:
    DRIVE_ROOT = pathlib.Path(_DRIVE_ROOT_CFG).expanduser().resolve()
else:
    # Default: Colab Drive mount point
...
# Apply DRIVE_ROOT to downstream modules via env (fallback if not passed explicitly)
