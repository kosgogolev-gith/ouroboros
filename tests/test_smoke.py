...
with patch.dict('sys.modules', {'ouroboros.supervisor.telegram_gateway': MagicMock(TelegramGateway=MockTelegramGateway)}):
    ...
    mock_tool_context = ToolContext(
        ...
        tg=mock_tg_instance, # Pass the mock TelegramGateway instance
    )

    # Initialize ToolRegistry with the mock ToolContext
    try:
        tool_registry = ToolRegistry(repo_dir=Path("/tmp/repo"), drive_root=Path("/tmp/drive"), tg=mock_tg_instance)
